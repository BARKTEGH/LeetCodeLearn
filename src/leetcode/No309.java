 ...
 *
 * 本文截取自公众号 labuladong 的文章：团灭 LeetCode 股票问题。力扣 markdown 的图片链接容易出问题，建议点击链接获得更好的阅读体验。
 *
 * 上篇文章 一种通用思路 用递归的方法实现了一套简单易懂的可行解，但是时间复杂度略高，不能通过全部测试用例，这篇文章用状态机的技巧来解决，可以全部提交通过。不要觉得这个名词高大上，文学词汇而已，实际上就是 DP table，看一眼就明白了。
 *
 * 先随便抽出一道题，看看别人的解法：
 *
 ...
 *
 * 如果你还不熟悉题目，可以去 LeetCode 或前文「一种通用思路」查看这些题目的内容，本文为了节省篇幅，就不列举这些题目的具体内容了。下面言归正传，开始解题。
 *
 * 一、穷举框架
 *
 ...
 *     if (i - 1 == -1) { }
        ...
        *
        *这里 k 取值范围比较小，所以可以不用 for 循环，直接把 k=1和 2的情况手动列举出来也可以：
        *
        ...
        *第六题，k=any integer
        *
        *有了上一题 k=2的铺垫，这题应该和上一题的第一个解法没啥区别。但是出现了一个超内存的错误，原来是传入的 k 值会非常大，dp 数组太大了。现在想想，交易次数 k 最多有多大呢？
        *
        ...
        *四、最后总结
        *
        *本文给大家讲了如何通过状态转移的方法解决复杂的问题，用一个状态转移方程秒杀了 6道股票买卖问题，现在想想，其实也不算难对吧？这已经属于动态规划问题中较困难的了。
        *
        ...
        *
        *对你有帮助的话，欢迎关注公众号 labuladong，鼓励一下我。
        *
        *作者：labuladong
        *链接：https://leetcode-cn.com/problems/two-sum/solution/yi-ge-tong-yong-fang-fa-tuan-mie-6-dao-gu-piao-w-3/
        *来源：力扣（LeetCode）
